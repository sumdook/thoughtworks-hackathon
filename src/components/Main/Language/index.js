...
class Language extends Component{
    ...
    renderCarousel = () => {
        ...
        if(currentLanguage){
            const movieList = this.props.util.languages[currentLanguage];
    ...
    renderGenre = () => {
        if(Object.keys(this.props.movies).length){
            const langList = this.props.util.languages;
            return Object.keys(langList).map(item=>{
                return(
                    <Col span={6}>
                    
                            <Card
                            onClick={()=>{this.setState({currentLanguage:item})}}
                            hoverable
                            style={{width: 350, margin:'10px', borderColor:'rgba(219, 24, 24, .2)', borderRadius:10, textAlign:'center'}}
                        >
                            <span className="genre-span">{item||"Unknown"}</span>
                            <p>{this.props.util.languages[item].length} movies</p>
                        </Card>
                    </Col>
               ); 
            })
    ...
    render(){
        ...
        return(
            <div>
                <h1 className="text-header">Choose your <span className="text-span"></span></h1>
                <Slider {...settings}>
                    {this.renderGenre()}
                </Slider>
                <p className="text-header text-span">{this.state.currentLanguage}</p>
                <Slider {...settings}>
                    {this.renderCarousel()}
                </Slider>
            </div>
